...
sub decode_bech32 {
    ...
    for ($p = 0; $p < scalar @bech32_encoded; ++$p) {
        ...
        die "Cannot decode bech32 string: One or more characters are improper unicode! pos: $p"
...
sub decodebech32tohex{
    ...
    #Converts binary to hexadecimal.
    my $hex_binary_address = '';
    foreach(@decoded_hex_data){
            #$hex_binary_address .= sprintf("%02X", $_);
            $hex_binary_address .= $_;
    }
...
#Bech32 test.
my $bech32_encoded_addr = "bc1qzpasslyy0jytpkf735n9fqsgra96xsums8eksc";
print "Running test for bech32 address $bech32_encoded_addr\n";
#print "Valid ", check_bitcoin_address($bech32_encoded_addr), ". For Bech32 encode/decode visit:https://slowli.github.io/bech32-buffer/\n\n";
my $bech32_binary_addr = decodebech32tohex($bech32_encoded_addr);
print "Original hex binary address: $bech32_binary_addr\n";



my $fileSrc = 'bech32.txt';
open my $fhSrc, $fileSrc or die "Could not open $fileSrc: $!";

my $fileDest = 'hex_bech32.txt';
open(my $fhDest, '>>', $fileDest) or die "Could not open file $fileDest: $!";
while( my $bech32_address = <$fhSrc>)  {
  my $addr_len = length($bech32_address);
  if ($addr_len == 43){
      $bech32_address = substr($bech32_address, 0, 42);
  }else{
      $bech32_address = substr($bech32_address, 0, 62);      
  }
  my $binary_address = decodebech32tohex($bech32_address);
